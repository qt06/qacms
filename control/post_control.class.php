...
class post_control extends common_control {
	...
	public function on_post() {
		$fid =  core::gpc('fid', 'P') ?  intval(core::gpc('fid', 'P')) : intval(core::gpc('fid'));
		$tid =  core::gpc('tid', 'P') ?  intval(core::gpc('tid', 'P')) : intval(core::gpc('tid'));
		$quickpost = core::gpc('quickpost', 'P') ? intval(core::gpc('quickpost', 'P')) : intval(core::gpc('quickpost'));
		...
		if(!$this->form_submit()) {
			...
			$pid =  core::gpc('pid', 'P') ?  intval(core::gpc('pid', 'P')) : intval(core::gpc('pid'));
			if($pid) {
			...
			if(!array_filter($error)) {
				...
				//加入到thread_new表
				$thread_new = $this->thread_new->read($tid);
				if($thread_new) {
					$thread_new['lastpost'] = $_SERVER['time'];
					$this->thread_new->update($thread_new);
				} else {
					$this->thread_new->create(array('tid'=>$tid,'fid'=>$fid,'dateline'=>$thread['dateline'],'lastpost'=>$_SERVER['time']));
				}
