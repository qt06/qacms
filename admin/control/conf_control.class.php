...
class conf_control extends admin_control {
	...
	public function on_base() {
		...
		if($this->form_submit()) {
			...
			$post['app_description'] = core::gpc('app_description', 'P');
			$post['homepage_type'] = core::gpc('homepage_type', 'P');
			...
			$post['post_post_expiry'] = intval(core::gpc('post_post_expiry', 'P'));
			...
			$post['header_toolbar'] = core::gpc('header_toolbar', 'P');
			...
			if(!array_filter($error)) {
				...
				foreach(array('app_name', 'app_description', 'homepage_type', 'app_copyright', 'seo_title', 'seo_keywords', 'seo_description', 
					'threadlist_hotviews', 'post_update_expiry', 'post_post_expiry', 'china_icp', 'header_toolbar', 'footer_js', 'site_pv', 'site_runlevel', 'forum_index_pagesize','reg_on', 'search_type'
					) as $k) {
		...
		$input['app_description'] = form::get_text('app_description', htmlspecialchars($kvconf['app_description']), 300);
		$input['homepage_type'] = form::get_radio('homepage_type', array('article'=>'文章模式', 'bbs'=>'论坛模式', 'blog'=>'博客模式'), $kvconf['homepage_type']);
		...
		$input['post_post_expiry'] = form::get_text('post_post_expiry', $kvconf['post_post_expiry'], 50);
		...
		$input['header_toolbar'] = form::get_text('header_toolbar', htmlspecialchars($kvconf['header_toolbar']), 300);
