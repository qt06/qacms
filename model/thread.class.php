...
class thread extends base_model {
	...
	// 按照 tid 倒序，获取最新的列表
	public function get_newlist($start = 0, $limit = 30, $orderby = 'lastpost', $threadlist = array()) {
		// 递归深度不能超过4次
		static $deep = 1;
		if($deep++ > 4) return $threadlist;
		
		$newlist = array();
		$newlist = $this->thread_new->index_fetch(array(), array($orderby=>-1), $start, $limit);
		foreach($newlist as $new) {
			$thread = $this->read($new['fid'], $new['tid']);
			$threadlist['thread-fid-' . $thread['fid'] . '-tid-' . $thread['tid']] = $thread;
		}
	...
	public function format(&$thread, $forum = array()) {
		...
		if($thread['top'] > 0) {
			...
			$thread['ico_desc'] = $thread['digest'] . '级精华';
		...
		$thread['lastpage'] = $thread['posts'] > $this->conf['pagesize'] ? ceil($thread['posts'] / $this->conf['pagesize']) : 1;
