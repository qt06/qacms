...
class base_model {
	...
	*/
	public function maxid($val = FALSE) {
		// FRAMEWORK_MAXID_SAFE 确保 maxid 为唯一的值。
		// fopen, flock, 100000 次，1 秒，效率还可以。
		if(FRAMEWORK_MAXID_SAFE && is_string($val)) {
			$lockfile = $this->conf['tmp_path'].$this->conf['app_id'].$this->table.'.maxid.lock';
			$fp = fopen($lockfile, 'w+');
			$i = 0;
			while(!flock($fp, LOCK_EX)) {
				if(++$i > 10) {
					throw new Exception('Database is too busy, get maxid failed.');
				}
				usleep(20000); // 20 毫秒 = 20000 微秒，如果这个函数被禁用，直接报错，不予处理。
			}
			$n = $this->db_cache_maxid($val);
			fclose($fp); // 关闭句柄，自动解锁。
			return $n;
		} else {
			$n = $this->db_cache_maxid($val);
